...
import { updateVariable } from "../../app/variables/actions";
import type { ManualEditState } from "../../app/variables/types";
...
export function DesignVariableEditor({ externalKey, mode, unit, value }: Props) {
  ...
  const inline = mode?.kind === "inline";
  const formId = mode?.kind === "inline" ? mode.formId : undefined;
  const initialFocus = mode?.kind === "inline" ? mode.initialFocus : undefined;
  const onDirtyChange = mode?.kind === "inline" ? mode.onDirtyChange : undefined;
  const onSaved = mode?.kind === "inline" ? mode.onSaved : undefined;
