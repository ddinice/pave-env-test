...
export default async function VariablesPage({
  ...
  const sort = validSorts.includes(params.sort as (typeof validSorts)[number])
    ? (params.sort as (typeof validSorts)[number])
    : "name";
  const variables = await listDesignVariables({
    query: params.query,
    subsystem: params.subsystem,
    sort,
  });
  const subsystems = [
    ...new Set(variables.map((variable) => variable.subsystem)),
  ].sort();
