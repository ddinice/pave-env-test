...
import { DesignVariableEditor } from "../../../components/DesignVariableEditor/design-variable-editor";
...
export default async function VariableDetailPage({
  ...
  params: Promise<{ externalKey: string }>;
  ...
  return (
    <AppShell userName={user.name}>
      ...
      <section className="variable-detail" aria-labelledby="variable-heading">
        <div className="variable-primary">
          <p className="eyebrow">{variable.subsystem}</p>
          <h1 id="variable-heading">{variable.name}</h1>
          ...
          <div className="current-value">
            ...
            <strong>
              {variable.value} <small>{variable.unit}</small>
            </strong>
          </div>
        </div>
        <aside className="variable-context" aria-label="Variable context">
          {variable.isProtected ? (
            <Badge className="status-badge">Protected</Badge>
          ) : (
            <span className="standard-access">Standard access</span>
          )}
      ...
      {editable ? (
        <section className="edit-panel" aria-labelledby="edit-heading">
          <div>
            ...
            <h2 id="edit-heading">Edit value</h2>
            <p>
              Changes update the registry immediately and record you as the
              editor.
            </p>
          </div>
        ...
        <section
          ...
          aria-label="Protected variable access"
        >
          <Badge className="status-badge">Protected</Badge>
          <p>
            This is a protected variable. Only an Engineering Lead can edit it.
          </p>
        </section>
      )}
    </AppShell>
